...
export interface FormDef {
  ...
  unitFields?: string[];
  /**
   * Drawn by code rather than stamped onto a scan. There is no template image
   * and no field to position, so the mapper explains that instead of offering a
   * canvas — it is listed only so a recruiter looking for it finds an answer.
   */
  generated?: boolean;
  /**
   * Where each field starts before anyone has placed it.
   *
   * A form measured off its scan can ship an approximate mapping, so the first
   * recruiter to open it nudges boxes into place instead of hunting for a
...
FORMS.push({
  ...
  paragraphs: [],
  retired: {},
  note: "Unbranded, generated by the app — no agency name, no template, nothing to place.",
});

FORMS.push({
  id: "id988a",
  label: "ID 988A Visa Form",
  pages: 2,
  width: ID988A_PAGE_W,
  height: ID988A_PAGE_H,
  sections: ID988A_SECTIONS,
